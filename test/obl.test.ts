...
describe("OBL Support", () => {
  ...
  test("should maintain bidirectional mapping (OBL -> History -> OBL)", () => {
    ...
    const roundTripOBL = OblUtil.fromHistoryEntries(
      entries,
      "test-user",
      "test-source",
    );
    ...
    const uttEvent = roundTripOBL.sessions[0].events.find(
      (e) => e.type === "utterance",
    );
    ...
    const actionEvent = roundTripOBL.sessions[0].events.find(
      (e) => e.type === "action",
    );
  ...
  test("should parse real OBLA data from dataset", () => {
      ...
      sessions: [
        {
          ...
          events: [
            ...
            },
            {
              ...
              vocalization: "Hello",
              actions: [
                { action: ":open_board", destination_board_id: "board-2" },
              ],
            },
          ],
          anonymized: true,
    ...
    const totalOriginalEvents = parsed.sessions.reduce(
      (acc, s) => acc + s.events.length,
      0,
    );
    const roundTrip = OblUtil.fromHistoryEntries(
      history,
      parsed.user_id,
      parsed.source,
    );
  ...
  test("bulk test real OBLA files (first 10)", () => {
    ...
    if (!fs.existsSync(oblaDir)) {
      console.warn(
        "Skipping bulk OBLA test - test/assets/obla directory not found",
      );
      return;
    }
