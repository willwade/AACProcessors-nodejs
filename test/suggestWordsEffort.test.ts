...
import {
  EFFORT_CONSTANTS,
  visualScanEffort,
} from "../src/utilities/analytics/metrics/effort";
...
function buildTreeWithPredictions(
  ...
  const btn = new AACButton({
    ...
    predictions,
    pos,
    parameters: parametersPredictions
      ? { predictions: parametersPredictions }
      : undefined,
  });
...
describe("Suggest Words effort cost", () => {
  ...
  it("does not add confirmation cost to morphology word forms", () => {
    ...
    expect(goes!.effort).toBeCloseTo(
      parentBtn!.effort + visualScanEffort(0),
      4,
    );
  });
  ...
  it("only adds confirmation to Suggest Words forms when predictions are mixed", () => {
    ...
    const { tree } = buildTreeWithPredictions(
      allPredictions,
      suggestWordsOriginals,
      "Noun",
    );
    ...
    expect(somes!.effort).toBeCloseTo(
      parentBtn!.effort + visualScanEffort(2),
      4,
    );
  });
  ...
  it("SUGGEST_WORDS_SELECTION_EFFORT is between 0.5 and 1.0", () => {
    ...
    expect(EFFORT_CONSTANTS.SUGGEST_WORDS_SELECTION_EFFORT).toBeLessThanOrEqual(
      1.0,
    );
  });
});
