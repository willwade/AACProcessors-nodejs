...
describe('SnapProcessor Audio Support', () => {
  const exampleSPSFile = path.join(__dirname, '../examples/Aphasia Page Set With Sound.sps');
  const enhancedSPSFile = path.join(__dirname, '../Aphasia_Page_Set_With_Punjabi_Audio.sps');
  ...
  it('should extract buttons for audio processing', () => {
    ...
    try {
      ...
      if (pages.length > 0) {
        ...
        if (pageWithButtons) {
          const buttons = processor.extractButtonsForAudio(exampleSPSFile, pageWithButtons.id);
  ...
  it('should add audio to buttons', () => {
    ...
    const processor = new SnapProcessor();
    const testDbPath = path.join(__dirname, 'test_audio_temp.sps');
    ...
    try {
      ...
      const testAudioData = Buffer.from('RIFF....WAVE....', 'ascii'); // Minimal WAV-like data
      
      // Add audio to a button (using button ID 1 as a test)
      const audioId = processor.addAudioToButton(testDbPath, 1, testAudioData, 'Test Audio');
  ...
  it('should load enhanced pageset with Punjabi audio', () => {
    ...
    const processor = new SnapProcessor(null, { loadAudio: true });
    const tree = processor.loadIntoTree(enhancedSPSFile);
