...
describe("CLI Comprehensive Tests", () => {
  ...
  describe("Command Parsing Tests", () => {
    ...
    it("should parse convert command with all options", () => {
      ...
      const result = execSync(
        `node ${cliPath} convert ${inputFile} ${outputFile} --format opml`,
        {
          encoding: "utf8",
          cwd: tempDir,
        },
  ...
  describe("File Processing Tests", () => {
    ...
    it("should convert OPML to DOT format", () => {
      ...
      execSync(
        `node ${cliPath} convert ${tempDotFile} ${opmlFile} --format opml`,
        {
          cwd: tempDir,
          stdio: "pipe",
        },
      ...
      // Convert back to DOT
      execSync(
        `node ${cliPath} convert ${opmlFile} ${finalDotFile} --format dot`,
        {
          cwd: tempDir,
          stdio: "pipe",
        },
  ...
  describe("Integration Tests", () => {
    ...
    it("should convert example.obf to dot format", () => {
      ...
      if (fs.existsSync(exampleObfFile)) {
        ...
        execSync(
          `node ${cliPath} convert ${exampleObfFile} ${outputFile} --format dot`,
          {
            cwd: tempDir,
            stdio: "pipe",
          },
  ...
  describe("Error Handling Tests", () => {
    ...
    it("should handle invalid output paths for convert command", () => {
      ...
      try {
        execSync(
          `node ${cliPath} convert ${inputFile} ${invalidOutputPath} --format opml`,
          {
            encoding: "utf8",
            cwd: tempDir,
            stdio: "pipe",
