...
async function auditGridsetImages(gridsetPath: string): Promise<AuditResult> {
  ...
  const resolvedImagesNotInZip = Array.from(resolvedImagePaths).filter(
    (img) =>
      !allEntries.includes(img) &&
      !allEntries.includes(img.replace(/^Grids\//, "")),
  );
  ...
  return {
    ...
    cellsWithDeclaredImages,
    cellsWithResolvedImages,
    cellsWithoutResolvedImages:
      cellsWithDeclaredImages - cellsWithResolvedImages,
...
describe("Gridset Image Audit", () => {
  const exampleGridset = path.join(
    process.cwd(),
    "examples/example-images.gridset",
  );
  ...
  test("should resolve all images that exist in the ZIP", async () => {
    ...
    console.log(
      `Cells without resolved images: ${audit.cellsWithoutResolvedImages}`,
    );
    console.log(`Actual image files in ZIP: ${audit.actualImageFilesInZip}`);
    console.log(
      `Unique resolved image paths: ${audit.resolvedImagePaths.length}`,
    );
    console.log(
      `Resolved images not found in ZIP: ${audit.resolvedImagesNotInZip.length}`,
    );
    ...
    if (audit.unresolvedCells.length > 0) {
      ...
      audit.unresolvedCells.forEach((cell) => {
        console.log(
          `  - "${cell.label}" at (${cell.x}, ${cell.y}): ${cell.imageName}`,
        );
      });
    }
    ...
    console.log("\n=== Summary ===");
    console.log(
      `✓ All ${audit.cellsWithResolvedImages} resolved images exist in the ZIP`,
    );
    console.log(
