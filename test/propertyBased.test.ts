...
describe('Property-Based Testing', () => {
  ...
  const aacButtonGenerator = fc.record({
    id: validIdGenerator,
  ...
  const aacTreeGenerator = fc.array(aacPageGenerator, { minLength: 1, maxLength: 10 })
    .map(pages => {
  ...
  describe('Round-Trip Property Tests', () => {
    it('DOT processor should preserve tree structure through round-trip', () => {
      fc.assert(fc.property(aacTreeGenerator, (originalTree) => {
        ...
        try {
          ...
          const originalPageCount = Object.keys(originalTree.pages).length;
          ...
          const originalPageNames = Object.values(originalTree.pages).map(p => p.name).sort();
          const reloadedPageNames = Object.values(reloadedTree.pages).map(p => p.name).sort();
          
          // At least some page names should be preserved
          const commonNames = originalPageNames.filter(name => 
            reloadedPageNames.some(reloadedName => 
              reloadedName.includes(name) || name.includes(reloadedName)
            )
          );
    ...
    it('OPML processor should preserve hierarchical structure', () => {
      fc.assert(fc.property(aacTreeGenerator, (originalTree) => {
        ...
        try {
          const outputPath = path.join(tempDir, `opml_roundtrip_${Date.now()}_${Math.random()}.opml`);
          processor.saveFromTree(originalTree, outputPath);
    ...
    it('OBF processor should preserve button structure', () => {
      fc.assert(fc.property(aacTreeGenerator, (originalTree) => {
        ...
        try {
          const outputPath = path.join(tempDir, `obf_roundtrip_${Date.now()}_${Math.random()}.obf`);
          processor.saveFromTree(originalTree, outputPath);
          ...
          const originalButtonCount = Object.values(originalTree.pages)
            .reduce((sum, page) => sum + page.buttons.length, 0);
          const reloadedButtonCount = Object.values(reloadedTree.pages)
            .reduce((sum, page) => sum + page.buttons.length, 0);
  ...
  describe('Translation Invariant Tests', () => {
    const translationMapGenerator = fc.dictionary(
      validLabelGenerator,
      validLabelGenerator,
      { maxKeys: 10 }
      ...
      fc.assert(fc.property(
        ...
        (content, translations) => {
          ...
          try {
            ...
            const dotContent = `digraph G {\n${content.split(' ').slice(0, 5).map((word, i) => 
              `  node${i} [label="${word}"];`
            ).join('\n')}\n}`;
            
            const outputPath = path.join(tempDir, `translation_test_${Date.now()}_${Math.random()}.dot`);
      ...
      fc.assert(fc.property(
        ...
        (content) => {
          ...
          try {
            ...
            const outputPath = path.join(tempDir, `empty_translation_${Date.now()}_${Math.random()}.dot`);
            
            const result = processor.processTexts(Buffer.from(dotContent), emptyTranslations, outputPath);
  ...
  describe('Data Structure Invariants', () => {
    it('AACTree should maintain page uniqueness', () => {
      fc.assert(fc.property(aacTreeGenerator, (tree) => {
    ...
    it('AACPage should maintain button ID uniqueness within page', () => {
      fc.assert(fc.property(aacPageGenerator, (page) => {
  ...
  describe('Text Extraction Properties', () => {
    it('Extracted texts should be non-empty strings', () => {
      fc.assert(fc.property(aacTreeGenerator, (tree) => {
        ...
        try {
          const outputPath = path.join(tempDir, `text_extraction_${Date.now()}_${Math.random()}.dot`);
    ...
    it('Text extraction should be deterministic', () => {
      fc.assert(fc.property(aacTreeGenerator, (tree) => {
        ...
        try {
          const outputPath = path.join(tempDir, `deterministic_${Date.now()}_${Math.random()}.dot`);
          processor.saveFromTree(tree, outputPath);
