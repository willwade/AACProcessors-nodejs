...
describe("Validation - additional formats", () => {
  ...
  it("validates Excel workbook buffers", async () => {
    ...
    const result = await validateFileOrBuffer(
      buffer,
      defaultFileAdapter,
      "sample.xlsx",
    );
  ...
  it("exposes ValidationResult on OPML parse failures", async () => {
    ...
    await expect(new OpmlProcessor().loadIntoTree(invalid)).rejects.toThrow(
      ValidationFailureError,
    );
  });
  ...
  it("exposes ValidationResult on DOT binary content", async () => {
    ...
    await expect(new DotProcessor().loadIntoTree(invalid)).rejects.toThrow(
      ValidationFailureError,
    );
  });
});
