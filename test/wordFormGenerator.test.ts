...
describe('WordFormGenerator', () => {
  ...
  describe('generateFromEngineSlots', () => {
    test('regular verb walk -> BASE + 3.PERS + PAST + GERUND', () => {
      ...
      const past = forms.find((f) => f.tags.includes('PAST') && !f.tags.includes('PARTICIPLE'));
    ...
    test('adjective big -> BASE + COMPARATIVE + SUPERLATIVE', () => {
      const forms = generator.generateFromEngineSlots('big', 'Adjective', engine);
    ...
    test('all forms have lang', () => {
      const forms = generator.generateFromEngineSlots('walk', 'Verb', engine, 'en');
    ...
    test('inflected forms have base set', () => {
      const forms = generator.generateFromEngineSlots('go', 'Verb', engine, 'en');
  ...
  describe('conditionsToTags', () => {
    ...
    test('returns UNKNOWN for unmapped conditions', () => {
      const tags = generator.conditionsToTags(new Map([['unknownDim', 'unknownVal']]));
  ...
  describe('end-to-end with synthetic XML', () => {
    test('walk via synthetic parser produces correct word forms', () => {
      ...
      const astericsForms = generator.generateFromGrid3Conditions('walk', walkForms!, 'en');
