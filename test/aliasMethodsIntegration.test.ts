...
describe("Alias Methods Integration", () => {
  ...
  describe("TouchChatProcessor Alias Methods", () => {
    ...
    it("should extract strings with metadata in expected format", async () => {
      ...
      const result: ExtractStringsResult =
        await processor.extractStringsWithMetadata(exampleFile);
      ...
      if (result.extractedStrings.length > 0) {
        ...
        expect(
          Array.isArray(firstString.vocabPlacementMeta.vocabLocations),
        ).toBe(true);
    ...
    it("should handle errors gracefully", async () => {
      ...
      const result =
        await processor.extractStringsWithMetadata(nonExistentFile);
  ...
  describe("Backward Compatibility", () => {
    ...
    it("should not break existing functionality", async () => {
      ...
      if (!fs.existsSync(exampleFile)) {
        console.log(
          "Skipping backward compatibility test - example file not found",
        );
        return;
      }
