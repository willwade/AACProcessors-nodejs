...
describe('SnapProcessor Coverage', () => {
  ...
  describe('Audio Handling', () => {
    it('should load audio data when loadAudio is true', () => {
      ...
      db.prepare('INSERT INTO Button (Id, Label, MessageRecordingId, ElementReferenceId) VALUES (?, ?, ?, ?)').run(1, 'Test', 1, 1);
      db.prepare('INSERT INTO PageSetData (Id, Identifier, Data) VALUES (?, ?, ?)').run(1, 'SND:123', Buffer.from('audio data'));
      db.close();
    ...
    it('should add audio to a button', () => {
        ...
        const audioRow = db.prepare('SELECT * FROM PageSetData WHERE Id = ?').get(row.MessageRecordingId) as any;
