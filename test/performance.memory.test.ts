...
describe('Memory Performance Tests', () => {
  ...
  function measureMemoryUsage<T>(operation: () => T): { result: T; memoryUsedMB: number; peakMemoryMB: number } {
  ...
  describe('TouchChatProcessor Memory Tests', () => {
    it('should process 1000+ button boards under 50MB memory', () => {
      ...
      const { result: tree, memoryUsedMB, peakMemoryMB } = measureMemoryUsage(() => {
      ...
      console.log(`TouchChat 1000+ buttons - Memory used: ${totalMemoryUsed.toFixed(2)}MB, Peak: ${peakMemoryMB.toFixed(2)}MB`);
    });
  ...
  describe('SnapProcessor Memory Tests', () => {
    it('should process 1000+ button boards under 50MB memory', () => {
      ...
      console.log(`Snap 1000+ buttons - Memory used: ${totalMemoryUsed.toFixed(2)}MB, Peak: ${peakMemoryMB.toFixed(2)}MB`);
    });
    ...
    it('should maintain memory usage under 100MB for large files', () => {
      ...
      const { result: tree, memoryUsedMB } = measureMemoryUsage(() => {
        ...
        Object.values(tree.pages).forEach((page, pageIndex) => {
          page.buttons.forEach((button, buttonIndex) => {
            if (buttonIndex % 3 === 0) { // Every 3rd button has audio
              button.audioRecording = {
                id: pageIndex * 100 + buttonIndex,
