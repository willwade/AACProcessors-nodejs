...
function ws(x: number, y: number, colSpan: number, rowSpan: number, subType = 'WebBrowser'): AACButton {
...
async function main() {
  ...
  const main = new AACPage({ id: 'web-browser', name: 'Web Browser', grid: { columns: 10, rows: 7 } });

  // Page title live cell (row 0, cols 1-8)
  main.addButton(new AACButton({
    id: 'wb-title',
    label: '',
    message: '',
    x: 1, y: 0, columnSpan: 8, rowSpan: 1,
  ...
  const view = new AACPage({ id: 'web-view', name: 'Web Browser view', grid: { columns: 10, rows: 7 } });
  view.addButton(ws(1, 1, 8, 6));

  view.addButton(new AACButton({
    id: 'wv-back',
    label: 'Back',
    message: 'Back',
    x: 0, y: 0, rowSpan: 2,
  ...
  view.addButton(new AACButton({
    ...
    semanticAction: {
      ...
      platformData: { grid3: { commandId: 'WebBrowser.ReadingMode', parameters: { action: 'Toggle' } } },
    },
  }));
  ...
  view.addButton(new AACButton({
    ...
    semanticAction: {
      category: AACSemanticCategory.SYSTEM_CONTROL,
      intent: AACSemanticIntent.WEB_SCROLL,
      platformData: { grid3: { commandId: 'WebBrowser.ScrollUp', parameters: { size: 'Small' } } },
    },
  }));
  ...
  view.addButton(new AACButton({
    ...
    semanticAction: {
      category: AACSemanticCategory.SYSTEM_CONTROL,
      intent: AACSemanticIntent.WEB_SCROLL,
      platformData: { grid3: { commandId: 'WebBrowser.ScrollDown', parameters: { size: 'Small' } } },
    },
  }));
  ...
  const search = new AACPage({ id: 'web-search', name: 'Web Browser - search', grid: { columns: 10, rows: 7 } });
  search.addButton(ws(1, 1, 8, 4));

  search.addButton(new AACButton({
    id: 'ws-home',
    label: 'Home',
    message: 'Home',
    x: 0, y: 0, rowSpan: 2,
  ...
  search.addButton(new AACButton({
    ...
    semanticAction: {
      ...
      platformData: { grid3: { commandId: 'WebBrowser.NavigateUrl', parameters: { url: 'https://en.wikipedia.org' } } },
    },
  }));
  ...
  const url = new AACPage({ id: 'web-url', name: 'Web Browser - URL', grid: { columns: 10, rows: 7 } });
  // Small workspace at top to show current page
  url.addButton(ws(1, 1, 8, 2));
  // Address bar workspace
  url.addButton(new AACButton({
    id: 'wu-address',
    label: '',
    message: '',
    x: 1, y: 3, columnSpan: 8, rowSpan: 1,
    contentType: 'Workspace',
  ...
  const kb = new AACPage({ id: 'web-keyboard', name: 'Web Browser - keyboard', grid: { columns: 10, rows: 7 } });
  kb.addButton(ws(1, 0, 8, 2));

  kb.addButton(new AACButton({
    id: 'wk-home',
    label: 'Home',
    message: 'Home',
    x: 0, y: 0,
  ...
  const favs = new AACPage({ id: 'web-favourites', name: 'Web Browser - favourites', grid: { columns: 10, rows: 7 } });
  favs.addButton(ws(1, 1, 8, 4));

  favs.addButton(new AACButton({
    id: 'wf-home',
    label: 'Home',
    message: 'Home',
    x: 0, y: 0, rowSpan: 2,
  ...
  favSites.forEach(site => {
    favs.addButton(new AACButton({
      ...
      semanticAction: {
        ...
        intent: AACSemanticIntent.WEB_NAVIGATE,
        text: site.url,
        platformData: { grid3: { commandId: 'WebBrowser.NavigateUrl', parameters: { url: site.url } } },
      },
    }));
  });

  // --- Links page (AutoContent links + workspace behind) ---
  const links = new AACPage({ id: 'web-links', name: 'Web Browser - links', grid: { columns: 10, rows: 7 } });
  links.addButton(ws(1, 1, 8, 4));

  // AutoContent for links
  links.addButton(new AACButton({
    id: 'wl-autolinks',
    label: '',
    message: '',
    x: 1, y: 5, columnSpan: 8, rowSpan: 2,
  ...
  const nav = new AACPage({ id: 'web-navigate', name: 'Web Browser - navigate', grid: { columns: 10, rows: 7 } });
  nav.addButton(ws(1, 1, 8, 6));

  nav.addButton(new AACButton({
    id: 'wn-home',
    label: 'Home',
    message: 'Home',
    x: 0, y: 0, rowSpan: 2,
  ...
  for (const [id, page] of Object.entries(reloaded.pages)) {
    const wsBtns = page.buttons.filter(b => b.contentType === 'Workspace' || b.contentType === 'LiveCell');
    console.log(`\n  "${page.name}" (${id}): ${page.buttons.length} buttons, ${wsBtns.length} workspace/livecell`);
    for (const btn of wsBtns) {
      console.log(`    WORKSPACE: ${btn.contentType}/${btn.contentSubType} @ (${btn.x},${btn.y}) span=(${btn.columnSpan}x${btn.rowSpan})`);
    }
  }
}
