...
export class ObfValidator extends BaseValidator {
  ...
  private async validateBoardStructure(board: any): Promise<void> {
    ...
    await this.add_check("locale", "locale", async () => {
      if (!board.locale) {
        this.err(
          'locale attribute is required, please set to "en" for English',
        );
      }
    });
    ...
    await this.add_check("grid", "grid attribute", async () => {
      ...
      if (
        !board.grid.order.every(
          (r: any) => Array.isArray(r) && r.length === board.grid.columns,
        )
      ) {
        this.err(
    ...
    await this.add_check(
      "grid_ids",
      "button IDs in grid.order attribute",
      async () => {
        ...
        const unusedIds = buttonIds.filter(
          (id: any) => !usedButtonIds.includes(id),
        );
        ...
        await this.add_check(
          ...
          async () => {
            ...
            if (!image.url && !image.data && !image.symbol && !image.path) {
              this.err(
                "image must have data, url, path or symbol attribute defined",
              );
            }
      ...
      for (let i = 0; i < board.sounds.length; i++) {
        ...
        await this.add_check(
          ...
          async () => {
            ...
            if (
              !sound.content_type ||
              !sound.content_type.match(/^audio\/.+$/)
            ) {
  ...
  private async validateManifest(manifest: any, zip: JSZip): Promise<void> {
    ...
    await this.add_check(
      ...
      async () => {
        ...
        if (
          ...
          typeof manifest.paths.boards !== "object"
        ) {
    ...
    await this.add_check(
      "manifest_extras",
      "manifest.json extra attributes",
      async () => {
      ...
      for (const [id, boardPath] of Object.entries(manifest.paths.boards)) {
        await this.add_check(
          ...
          async () => {
            ...
            if (!bFile) {
              this.err(
                `board path (${boardPath}) not found in the zip package`,
              );
              return;
            }
            try {
        ...
        await this.add_check(
          ...
          async () => {
            if (!zip.file(soundPath as string)) {
              this.err(
                `sound path (${soundPath}) not found in the zip package`,
              );
            }
          },
        );
